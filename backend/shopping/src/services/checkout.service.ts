...
import type { CartItem, CartItemIds, CartItemInput, CartResponse, Order, OrderResponse, OrderItemIds } from "../index";
...
export async function addToCart(pool: Pool, userId: string, input: CartItemInput): Promise<Result<CartItem>> {
  ...
  try {
    ...
    const items = await client.query(`INSERT INTO "cartItems" ("cartId", "plantId", quantity) VALUES ($1, $2, $3) ON CONFLICT ("cartId", "plantId") DO UPDATE SET quantity = "cartItems".quantity + EXCLUDED.quantity WHERE "cartItems".quantity + EXCLUDED.quantity <= $4 RETURNING id, "cartId", "plantId", quantity`,
      [cartId, plantId, quantity, plantRow.stock]
    );
