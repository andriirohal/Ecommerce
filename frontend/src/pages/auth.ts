...
export async function refreshAccountSummary(): Promise<void> {
  ...
  await loadAccountSummary(user.userId, true);
};
...
export function renderLogin(): string {
  return `
    <main class="auth-page">
      <section class="auth-hero container">
        <div class="auth-card">
...
export function renderAccount(): string {
  ...
  const summary =
    user && accountSummary && summaryUserId === user.userId
      ? accountSummary
      : null;
      ...
      >
        ...
        >
          ...
          <div class="account-popover-header">
            ...
            <button
              type="button"
              class="account-popover-close"
              data-account-close
              aria-label="${t("header.accountPopover.close")}"
            >
              <span
                class="account-close-icon"
                aria-hidden="true"
              ></span>
            </button>
          </div>
...
export function initAccountActions(): void {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (logoutButton) {
      ...
      const loggedOutUserId = getCurrentUser()?.userId ?? null;
...
export function updateAccountUI(): void {
  ...
  if (accountSummary && summaryUserId === user.userId) {
...
export function initAccountUI(): void {
  ...
  window.addEventListener("auth-changed", () => {
    ...
    void loadAccountSummary(currentUser.userId, true).then(() => {
      updateAccountUI();
    });
  });
