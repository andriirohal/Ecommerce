  ...
  return `
    <div class="cart-layout">
      ...
      <aside class="summary-panel">
        ...
        <div class="shipping-status ${shipping === 0 ? "free-shipping" : ""}">
          <p
            class="hint"
...
function renderCart(lines: CartLine[]): string {
...
function updateCartSummary(root: HTMLElement): void {
  ...
  const shippingProgress =
    root.querySelector<HTMLElement>(
      "[data-cart-shipping-progress] .shipping-progress-bar"
    );
...
function initCartEvents(): void {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (
        target.closest(
          "button, a, input, select, textarea, .qty-control"
        )
      ) {
        return;
      };
